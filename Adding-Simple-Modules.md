...
Next the binary is called, `/usr/bin/dnsx`. Always be sure to add the full path to the binary for all modules. You can find the path with the command `which`. For example,`which dnsx`.
