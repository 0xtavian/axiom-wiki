You can install tools either before or after spinning up your fleet (monkey patching). If you already have a fleet, simply execute `axiom-exec`. This also works for updating. For example, to update templates Nuclei run `axiom-exec "nuclei -update-templates"`. To install a package with apt-get execute `axiom-exec "sudo apt-get install [package-name]"`
